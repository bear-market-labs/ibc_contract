...
contract InverseBondingCurveRouter {
    ...
    function _payAndExecute(address recipient, address pool, bool useNative, CommandType command, IERC20 reserveToken, IERC20 inverseToken, bytes memory data) private {
    ...
    function _getInputAndCallData(CommandType command, bytes memory data) private view returns (uint256 reserveTokenAmount, uint256 inverseTokenAmount, bytes memory poolCallData) {
        
        if (command == CommandType.ADD_LIQUIDITY) {
            (address recipient, uint256 reserveIn, uint256 minPriceLimit) = abi.decode(data, (address, uint256, uint256));
            reserveTokenAmount = reserveIn;
            poolCallData =
                abi.encodeWithSignature("addLiquidity(address,uint256,uint256)", recipient, reserveIn, minPriceLimit);
