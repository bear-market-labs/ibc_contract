...
contract InverseBondingCurveRouter {
    ...
    function _getCallDate(CommandType command, bytes memory data) private view returns (bytes memory poolCallData) {
        if(command == CommandType.ADD_LIQUIDITY){
            ...
            "addLiquidity(address,uint256,uint256)", msg.sender, reserveIn, minPriceLimit);
            ...
            (, uint256 reserveIn, uint256 exactAmountOut, uint256 maxPriceLimit) = abi.decode(data, (address, uint256, uint256, uint256));
            poolCallData = abi.encodeWithSignature(
            ...
            poolCallData = abi.encodeWithSignature(
            "claimReward(address)", msg.sender);
        }else{
            revert CommandUnsupport();
        }

        //TODO: also add stake/unstake here
